...
from .serializers import NetworkAssetSerializer, NetworkFindingSerializer


class NetworkAssetViewSet(OrganizationQuerySetMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return self.filter_by_organization(NetworkAsset.objects.all(), self.request.user)


class NetworkFindingViewSet(OrganizationQuerySetMixin, viewsets.ReadOnlyModelViewSet):
    serializer_class = NetworkFindingSerializer
    permission_classes = [IsAuthenticated, OrganizationAccessPermission]
    required_roles = [UserOrganization.ROLE_SECURITY_LEAD]
    organization_field = "network_asset__organization"

    def get_queryset(self):
        queryset = self.filter_by_organization(NetworkFinding.objects.all(), self.request.user).order_by("-created_at")
    ...
    def resolve(self, request, pk=None):
        ...
        if finding.status != NetworkFinding.STATUS_RESOLVED:
            finding.status = NetworkFinding.STATUS_RESOLVED
